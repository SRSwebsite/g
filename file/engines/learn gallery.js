...
const gasUrl = 'https://script.google.com/macros/s/AKfycbzKvlhYsMBYB_PR3saIBIe82PLXgnFKngroemuqSkU33hQn8CdozC0H1ubDMYwzTug60Q/exec';
...
levelIcon.style.marginLeft = '8px';

if (item.level.includes('پایه')) {
    levelIcon.className = 'fas fa-cube';
    levelIcon.style.color = '#6c757d';
} else if (item.level.includes('ورکشاپ')) {
    levelIcon.className = 'fas fa-tools';
    levelIcon.style.color = '#ff9800';
} else if (item.level.includes('VIP')) {
    levelIcon.className = 'fas fa-crown';
    levelIcon.style.color = '#ffd700';
}


    title.appendChild(levelIcon);
}

// متن عنوان دوره
const titleText = document.createTextNode(`عنوان دوره: ${item.name}`);
title.appendChild(titleText);
                ...
                button.textContent = 'مشاهده دوره';
